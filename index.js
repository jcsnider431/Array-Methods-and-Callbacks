...
function getYears(finalsCB) {
   const years = [];
   finalsCB.forEach(function(item){
...
function getWinners(data, getFinalsCB) {
    // const winners = [];
    // finalsCB.forEach(function(item){
    //     if(finalsCB['Home Team Goals'] > finalsCB['Away Team Goals']){
 ...
 */
 ...
 function getWinnersByYear(data, year, winnersCB) {
    let winnersbyyear = [];
    let winners = winnersCB(data);
    let years = year(data);
    winners.forEach(function(item,index){
        winnersbyyear.push(`In ${years[index]}, ${/*winners[index]*/ item} won the world cup!`);
    });
